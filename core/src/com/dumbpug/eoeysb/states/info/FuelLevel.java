 ...
 */
public class FuelLevel extends com.dumbpug.eoeysb.states.State {
    Pane pane;
    SpriteBatch batch;
    Sprite background;
    com.dumbpug.eoeysb.tools.Animation fuelLevelAnimation;

    com.dumbpug.eoeysb.tools.Button rightButton;
    com.dumbpug.eoeysb.tools.Button leftButton;
    com.dumbpug.eoeysb.tools.Button exitButton;

    // Aspect Ratio of controls animation
    float fuelLevellAnimationAspectRatio = 1.0847f;
    ...
    public void stateLoad() {
        ...
        // Set fuel level info animation.
        fuelLevelAnimation = new com.dumbpug.eoeysb.tools.Animation(new Texture(Gdx.files.internal("MiscImages/fuel_info_animation.png")), 1, 2, (float) 1/1f);

        // Set up our right button control
        rightButton = new com.dumbpug.eoeysb.tools.Button();
    ...
    public void renderState() {
        ...
        if(rightButton.isButtonReleased()){
            markStateChange(com.dumbpug.eoeysb.states.StateManager.STATE_INFO_FUEL_PODS, true, true);
        }
        // Get the current frame for our animation.
        TextureRegion currentAnimationFrame = fuelLevelAnimation.getCurrentFrame(true);
        int animationYPos = (int) ((Gdx.graphics.getHeight()/2) - ((Gdx.graphics.getWidth() * fuelLevellAnimationAspectRatio)/2));
        // Draw all elements.
        batch.begin();
        background.draw(batch);
        batch.draw(currentAnimationFrame, 0, animationYPos, Gdx.graphics.getWidth(), Gdx.graphics.getWidth() * fuelLevellAnimationAspectRatio);
        pane.drawPane(batch);
        batch.end();
    }
